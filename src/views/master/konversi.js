...
export default {
    ...
    async konversi (param) {
        store().loader('on')
        let head = await api.getData('/konversi_barang?status=true', this.parameters(param))
        let data = []
    ...
    async filtered(input) {
        ...
        store().$patch((state) => { state.state.filter[0].item = ft_kode})
    ...
    delete(data) {
        alert.confirm('Apakah anda yakin ?', 'Anda akan menghapus '+data.kode_konversi).then((result) => {
            if(result.isConfirmed) {
                api.delete('konversi_barang', {kode_konversi: data.kode_konversi}).then(result => {
