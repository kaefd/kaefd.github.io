    ...
    <v-card>
      ...
      <v-container class="h-100 d-flex flex-column mt-5">
        ...
        >
          <v-responsive class="pt-2 mx-md-0 mx-3" width="250">
            ...
            <text-field-form
              ...
              :model-value="functions.formatDate(dataitem.tgl_pengiriman)"
              readonly
            />
          </v-responsive>
        ...
        <v-form @submit.prevent ref="form" class="mx-3">
          <v-row v-if="!edit" no-gutters justify="center" justify-md="space-between">
            ...
            <v-responsive class="pt-2 mx-3" width="250">
              ...
              <text-field-form
                @click="dialogbongkar = true, more = 15"
                label="Tujuan Bongkar"
        ...
        <v-container v-if="!edit" class="text-sm-left text-center mt-n5 mb-n5">
          <dialogScroll
            :window="window"
            dialog_title="pengeluaran belum terkirim"
            @reset="reset"
            :nokirim="inputdata.no_pengiriman"
            :blmkirim="true"
            :pjl_blmterkirm="pjl_blmterkirm"
            :belumkirim="belumkirim"
            :belumkirim_detail="belumkirim_detail"
            :barang="barang"
          ...
          >
            ...
            <template v-slot:bottom>
              <v-span v-if="edit" class="float-end me-5 text-caption font-weight-medium"
                >Total Jumlah : {{ functions.numb(jumlahtotal(kirim_detail), 2, true) }}</v-span
              >
              <v-span v-if="!edit" class="float-end me-5 text-caption font-weight-medium"
                >Total Jumlah : {{ functions.numb(jumlahtotal(pembelian_input)) }}</v-span
              >
            </template>
            <!-- dialog actions -->
            <!-- CUSTOM KOLOM -->
    ...
    <v-dialog v-model="dialogbongkar" transition="dialog-bottom-transition" width="auto">
      <v-card class="py-5 px-7 rounded-xl mx-auto" min-width="300" height="90vh" max-width="400">
        <v-btn
          ...
          class="absolute"
          variant="text"
          @click="dialogbongkar = false"
        ></v-btn>
        <v-card-title class="text-center text-orange mb-3 text-button font-weight-bold"
          >ALAMAT BONGKAR</v-card-title
        >
        <v-div>
          <text-field v-model="searched" label="Search"></text-field>
        </v-div>
        <v-list class="my-4">
          <v-div class="vh-100">
            <v-list-item
              v-for="(s, i) in filteralamat.slice(0, more)"
              :key="i"
              density="compact"
              style="cursor: pointer"
              class="text-caption"
