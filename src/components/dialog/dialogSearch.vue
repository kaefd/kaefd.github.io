...
<v-dialog>
    <template v-slot:activator="{props}">
        ...
        <pillsButton v-if="button" v-bind="props" icon="mdi-plus" :btn_title="btn_title"/>
    </template>
    <v-card class="py-5 px-5 rounded-xl mx-auto w-100" max-width="400">
        <slot name="close"></slot>
        <v-card-title class="text-center text-blue-darken-4 mb-3 mt-n10 text-button font-weight-bold">{{ cardTitle }}</v-card-title>
        <textField
            v-model="search"
            label="Search"
        ></textField>
