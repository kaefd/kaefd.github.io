        ...
        <div class="w-full h-full overflow-x-auto md:overflow-y-hidden">
            <table id="table" class="w-full h-full text-xs text-left">
                <thead class="w-full flex bg-primary-hover text-center text-sm">
                    <tr class="w-full flex items-center" :class="fields.length > 1 ? 'justify-around' : 'justify-between'">
                        <th v-for="(field, f) in fieldCol" scope="col" class="w-[15%] min-w-[100px] px-5 py-3">
...
methods: {
    sorts(data, index) {
        ...
        if(data.type != 'number') {
            if(this.desc) this.items.sort((a, b) => b.head[data.key].localeCompare(a.head[data.key]))
            else this.items.sort((a, b) => a.head[data.key].localeCompare(b.head[data.key]))
        } else {
            if(this.desc) this.items.sort((a, b) => b.head[data.key] - (a.head[data.key]))
            else this.items.sort((a, b) => a.head[data.key] - (b.head[data.key]))
        }
    },
