  ...
  <v-dialog v-model="dialog" :scrim="false" transition="dialog-bottom-transition" fullscreen>
    ...
    <template v-slot:activator="{ props }">
      ...
      <v-menu>
        ...
        <v-list>
          <v-list-item class="text-caption" density="compact" v-bind="props"
            >Lihat Data</v-list-item
          >
          <v-list-item v-if="hapus" class="text-caption" density="compact" @click="confirm()"
            >Batal Produksi</v-list-item
          >
        </v-list>
      </v-menu>
    </template>
    ...
    <v-card>
      <v-toolbar class="bg-blue-custom text-white" height="50">
        <text-button
          icon="mdi-close"
          color="white"
          @click=";(dialog = false), (inputproduksi = inputbahan = inputbarang = [])"
        />
          ...
          >
            ...
            <v-responsive class="pt-2 mx-3 overflow-visible" width="250">
              ...
              <text-field-form
                ...
                :readonly="true"
                :rules="required"
                :model-value="functions.formatDate(item.tgl_produksi)"
              />
            </v-responsive>
            <v-responsive class="pt-2 mx-md-0 mx-3" width="250">
              <text-field-form
        ...
        >
          ...
          <v-responsive class="me-sm-2 me-0 text-sm-left text-center" width="400">
            ...
            <dialogScroll
              ...
              @pemasukanitem="bahanmasuk"
            />
            <v-row v-if="edit" no-gutters class="justify-center py-1 text-button rounded border"
              >detail bahan</v-row
            >
              ...
              >
                <template v-slot:bottom>
                  <v-span v-if="edit" class="float-end me-5 text-caption font-weight-medium"
                    >Jumlah Bahan : {{ functions.numb(jumlahtotal(detailbahan)) }}</v-span
                  >
                  <v-span v-if="!edit" class="float-end me-5 text-caption font-weight-medium"
                    >Jumlah bahan : {{ functions.numb(jumlahtotal(inputbahan)) }}</v-span
                  >
                </template>
                  ...
                  <DialogVue @update="dialogchild" v-model="dialogbhn[index]">
                    ...
                    <template #content>
                      ...
                      <v-div v-if="!edit" class="d-flex me-5 ms-auto">
                        <btn-cancel
                          btn_title="Hapus"
                          @click="deleteditem(item.raw, 'bahan'), (dialogbhn[index] = false)"
                          class="me-2"
                        ></btn-cancel>
                        <btn-orange
                          btn_title="Simpan"
          ...
          <v-responsive class="mt-md-0 mt-2 text-sm-left text-center" width="400">
            ...
            <v-container class="border-sm rounded-lg mt-2">
              ...
              >
                <template v-slot:bottom>
                  <v-span v-if="edit" class="float-end me-5 text-caption font-weight-medium"
                    >Jumlah Barang : {{ functions.numb(jumlahtotal(detailbarang)) }}</v-span
                  >
                  <v-span v-if="!edit" class="float-end me-5 text-caption font-weight-medium"
                    >Jumlah Barang : {{ functions.numb(jumlahtotal(inputbarang)) }}</v-span
                  >
                </template>
                <!-- eslint-disable-next-line vue/valid-v-slot -->
                <template v-slot:item.jumlah="{ item }">
                  ...
                  <DialogVue @update="dialogbrg" v-model="dialogbrg[index]">
                    ...
                    <template #content>
                      ...
                      <v-div v-if="!edit" class="d-flex me-5 ms-auto">
                        <btn-cancel
                          btn_title="Hapus"
                          @click="deleteditem(item.raw, 'barang'), (dialogbrg[index] = false)"
                          class="me-2"
                        ></btn-cancel>
                        <btn-orange
                          btn_title="Simpan"
                          type="submit"
                          @click="dialogbrg[index] = false"
                        ></btn-orange>
                      </v-div>
                    </template>
                  </DialogVue>
    ...
    <v-dialog v-model="dialog5" transition="dialog-bottom-transition" width="auto">
      <v-card
        class="py-5 px-7 rounded-xl mx-auto"
        min-width="300"
        :width="window < 600 ? '87vw' : '50vw'"
        height="90vh"
        max-width="400"
      >
        <v-btn
